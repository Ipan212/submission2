...
function addBook() {
  ...
  const inputAuthor = document.getElementById("penulis").value;
  const inputYear = document.getElementById("tanggal").value;

  const generatedID = generateId();
  const newBook = generateNewBook(generatedID, inputJudul, inputAuthor, inputYear, false);
  books.push(newBook);
...
function makeBook(newBook) {
  ...
  if (newBook.isReaded) {
    ...
    undoButton.setAttribute("src", "img/undo-outline.svg");
    undoButton.classList.add("undo-button");

    undoButton.addEventListener("click", function () {
      if (confirm("Oooh Jadi Bukunya Belum DIbaca Beres Dibaca Nih..?")) {
        undoBookTitleFromReaded(newBook.id);
        alert("Beresin Atuh Bacanya Yah..!");
      } else {
      }
    });
    ...
    trashButton.addEventListener("click", function () {
      if (confirm("Yakin Mau Dihapus Nih...?")) {
    ...
    checkButton.addEventListener("click", function () {
      if (confirm("Yakin Udah Beres Nih Bacanya..?")) {
...
resetList.addEventListener("click", function () {
  ...
  if (reset == 0) {
    ...
    if (confirm("Yakin Manu Hapus Semua Listnya?")) {
      resetBookList(reset);
    }
  }
});
